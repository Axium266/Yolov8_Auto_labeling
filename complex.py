...
# 兩個不同的 YOLO 模型
model1 = YOLO("C:/Users/USER/Desktop/all image/auto/light4.pt") 
model2 = YOLO("C:/Users/USER/Desktop/all image/auto/best.pt") 
...
CONF_THRESHOLD = 0.75  # 置信度閾值
IOU_THRESHOLD = 0.4    # IoU 閾值（去除重複標註）

# 重新對應 model 的標籤
remap_classes_model1 = {0: 8, 1: 9}  
    ...
    if image_name.endswith((".jpg", ".png", ".jpeg")):
        ...
        for result in results1:
            for box in result.boxes:
                cls = int(box.cls)  # 類別
                cls = remap_classes_model1.get(cls, cls)  # 重新映射類別 ID
                conf = float(box.conf)  # 置信度
                x_center, y_center, width, height = box.xywhn[0]  # YOLO 格式
